...
async function asignarTipoCuenta(userId){
  const select=document.getElementById(`account-type-${userId}`);
  if(!select) return;
  const role=select.value;
  const label=role==='abogado'?'abogado':'cliente';
  if(!confirm(`¿Asignar esta cuenta como ${label}? Esta acción habilitará su portal correspondiente.`)) return;
  try{
    await apiPost(`/admin/usuarios/${userId}/asignar-tipo`, { role });
    toast(`Portal ${label} asignado`);
    renderAdmin();
  }catch(err){ toast(err.error || 'No se pudo asignar el tipo de cuenta'); }
}
...
async function renderAdmin(){
  ...
  if(canAdmin()){
    document.getElementById('admin-usuarios').innerHTML = `<div class="admin-section-head"><div><h3>Usuarios</h3><p>Usuarios registrados, permisos internos y acceso directo a sus portales. Las cuentas sin tipo se pueden asignar aquí.</p></div><span class="pill pill-neutral">${users.length} cuentas</span></div><div class="admin-users-note"><strong>Portales:</strong> cada cuenta de cliente o abogado debe mostrar una acción visible. Las cuentas <strong>sin_definir</strong> requieren asignar Cliente o Abogado antes de abrir un portal.</div><div class="admin-users-table-wrap"><table><thead><tr><th>Nombre</th><th>Correo</th><th>Tipo</th><th>Rol interno</th><th>Estado</th><th>Créditos</th><th>Portal</th></tr></thead><tbody>${users.map(u=>{ const estado=u.role==='abogado'?(u.verified?'Verificado':'Pendiente'):u.role==='sin_definir'?'Tipo pendiente':'Activo'; const canPreview=['cliente','abogado'].includes(u.role); const portalAction=canPreview?`<button class="btn btn-outline btn-sm portal-user-btn" onclick="verPortalUsuario('${u._id}')">Abrir portal ${u.role==='abogado'?'abogado':'cliente'}</button>`:`<div class="admin-assign-portal"><select id="account-type-${u._id}" class="admin-role-select"><option value="cliente">Cliente</option><option value="abogado">Abogado</option></select><button class="btn btn-ink btn-sm" onclick="asignarTipoCuenta('${u._id}')">Asignar</button></div>`; return `<tr><td>${esc(u.name||'—')}</td><td>${esc(u.email)}</td><td>${u.role==='sin_definir'?'<span class="pill pill-brass">Tipo pendiente</span>':`<span class="pill pill-neutral">${esc(u.role)}</span>`}</td><td><span class="pill pill-neutral">${esc(u.staffRole||'none')}</span></td><td><span class="pill ${estado==='Verificado'||estado==='Activo'?'pill-forest':'pill-brass'}">${estado}</span></td><td class="mono">${u.role==='abogado'?u.credits:'—'}</td><td><div class="admin-preview-user-actions">${portalAction}</div></td></tr>`; }).join('')}</tbody></table></div>`;
