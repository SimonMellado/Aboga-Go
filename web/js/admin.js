...
async function revertirACliente(userId){
  if(!confirm('¿Volver esta cuenta a cliente? La verificación de abogado quedará reiniciada.')) return;
  try{ await apiPost(`/admin/usuarios/${userId}/cambiar-tipo`, { role:'cliente' }); toast('Cuenta cambiada a cliente'); renderAdmin(); }catch(err){ toast(err.error || 'No se pudo cambiar el tipo de cuenta'); }
}
...
async function renderAdmin(){
  let users=[], pendientes=[], sinCertificado=[], causas=[], transfers=[], purchases={ totals:{}, purchases:[] }, roles=[], security=null;
  try{
    const jobs=[];
    ...
    if(canReview()) jobs.push(apiGet('/admin/verificacion-pendiente').then(v=>pendientes=v), apiGet('/admin/abogados-sin-certificado').then(v=>sinCertificado=v));
  ...
  if(canAdmin()){
    ...
    if(hasPermission('cases_manage')) { let resumen={total:0,abiertas:0,proceso:0,cerradas:0,clientes:0,publicaciones30d:0,fallos24h:0,ultimas:[]}; try { resumen=await apiGet('/admin/causas/resumen-publicacion'); } catch(e) {} document.getElementById('admin-causas').innerHTML = `<div class="admin-section-head"><div><h3>Causas y salud de publicaciones</h3><p>Comprueba si los clientes están publicando realmente y si las causas están llegando al marketplace.</p></div><button class="btn btn-outline btn-sm" onclick="renderAdmin()">↻ Actualizar</button></div><div class="admin-case-health-grid"><div class="stat-card"><div class="lbl">Causas totales</div><div class="val">${Number(resumen.total||0)}</div></div><div class="stat-card"><div class="lbl">Disponibles</div><div class="val">${Number(resumen.abiertas||0)}</div></div><div class="stat-card"><div class="lbl">Tomadas</div><div class="val">${Number(resumen.proceso||0)}</div></div><div class="stat-card"><div class="lbl">Clientes activos</div><div class="val">${Number(resumen.clientes||0)}</div></div><div class="stat-card"><div class="lbl">Publicadas últimos 30 días</div><div class="val">${Number(resumen.publicaciones30d||0)}</div></div><div class="stat-card"><div class="lbl">Fallos de publicación · 24 h</div><div class="val">${Number(resumen.fallos24h||0)}</div></div></div>${resumen.total===0?'<div class="admin-users-note"><strong>No hay causas en la base de datos.</strong> Esto confirma que todavía no se ha publicado ninguna consulta, o que la publicación está fallando antes de guardarse. Revisa el contador de fallos y prueba el Portal cliente.</div>':resumen.abiertas===0&&resumen.proceso===0?'<div class="admin-users-note"><strong>No hay causas actualmente disponibles.</strong> Existen registros, pero ninguna está abierta o en proceso.</div>':''}<h4>Últimas publicaciones recibidas</h4>${resumen.ultimas?.length?`<table><thead><tr><th>N°</th><th>Cliente</th><th>Tipo</th><th>Comuna</th><th>Estado</th><th>Fecha</th></tr></thead><tbody>${resumen.ultimas.map(c=>`<tr><td class="mono">${esc(c.numero)}</td><td>${esc(c.client?.name||[c.client?.firstName,c.client?.lastName].filter(Boolean).join(' ')||c.client?.email||'—')}<br><span class="muted">${esc(c.client?.email||'')}</span></td><td>${esc(c.tipo)}</td><td>${esc(c.comuna)}</td><td><span class="pill ${c.status==='abierta'?'pill-forest':c.status==='en_proceso'?'pill-brass':'pill-neutral'}">${c.status==='abierta'?'Abierta':c.status==='en_proceso'?'En proceso':'Cerrada'}</span></td><td>${new Date(c.createdAt).toLocaleString('es-CL')}</td></tr>`).join('')}</tbody></table>`:'<div class="empty">Todavía no hay publicaciones registradas.</div>'}</div>`; }
...
async function probarCorreoResend(){ const el=document.getElementById('email-test-status'); if(el) el.textContent='Enviando prueba...'; try { const r=await apiPost('/admin/email/test',{}); if(el) el.textContent=r.message||'Correo de prueba enviado'; } catch(e){ if(el) el.textContent=e.error||'Falló la prueba. Revisa Render.'; } }
